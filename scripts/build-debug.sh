...
if ! flutter build apk --debug > /tmp/apk_build.log 2>&1; then
...
echo "🏗️  Building debug macOS app..."
if ! flutter build macos --debug > /tmp/macos_build.log 2>&1; then
...
if [ "$APK_SUCCESS" = true ]; then
    cp build/app/outputs/flutter-apk/app-debug.apk "/Users/tadeasfort/Desktop/meta-chat-viewer_debug.apk"
    echo "📱 Debug APK copied to Desktop"
fi

if [ "$MACOS_SUCCESS" = true ]; then
    cp -r build/macos/Build/Products/Debug/*.app "/Users/tadeasfort/Desktop/Meta Elysia debug.app"
    echo "🖥️  Debug macOS app copied to Desktop"
fi
